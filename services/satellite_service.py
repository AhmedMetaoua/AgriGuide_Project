"""
Section 6.1 (first slice) — vegetation index (NDVI) from Sentinel-2 via
the Copernicus Data Space Ecosystem Statistical API. This is real
satellite-derived data computed via band math, NOT a trained model —
the CNN land-cover/crop-type classifier (the rest of Section 6.1) is a
...
COPERNICUS_CLIENT_ID / COPERNICUS_CLIENT_SECRET in .env.
"""
import base64
import time
...
async def get_ndvi(geometry: dict, days_back: int = 30) -> VegetationData:
    ...
    if not settings.copernicus_client_id or not settings.copernicus_client_secret:
        return VegetationData(
            source="unavailable",
            warning=(
                "Copernicus credentials not set. Create a free account at "
...
# Colored NDVI PNG for the map overlay — same cloud/shadow/snow masking
# as _NDVI_EVALSCRIPT (SCL codes 3/8/9/10/11), but outputs an RGBA image
# via the Process API instead of a single aggregate stat via the
# Statistics API. Masked/no-data pixels are fully transparent (alpha 0)
# so the basemap shows through rather than a misleading solid color.
# Diverging red->yellow->green ramp, standard NDVI convention (low =
# stressed/bare, high = dense healthy vegetation).
...
async def get_band_timeseries(
    ...
    45-timestep input is dl_service.py's job (Day 5), done deterministically
