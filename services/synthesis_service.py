"""
Section 9 — two-stage generation. Stage 1 builds a grounded structured
JSON (every claim tagged with a source chunk or dropped). Stage 2 turns
that JSON into the French report. Kept as two separate Mistral calls,
not one — collapsing them loses the traceability guarantee.
...
Layer 3 history: the audit originally compared raw string tokens
("23,91 °C" vs "23.91") extracted from the French report against
tokens extracted from the JSON payload. A real end-to-end test showed
this fails on EVERY correctly-transcribed French decimal (comma vs
JSON's period, plus the unit suffix attached in prose but not in JSON)
— it was flagging area_ha and NDVI, untouched literal copies, as
"unverified". Rewritten below to parse both sides to actual float
values (normalizing comma->period, stripping units) and compare with
a small tolerance, which is what an audit checking correctness should
have done from the start.

Citation handling (## Conseils pratiques): previously asked Stage 2 to
write "(Source: {source_document})" itself — but Stage 2 was never
actually given source_document, only source_chunk_id (a raw UUID), so
it had no way to comply; the live report showed literal chunk UUIDs.
Fixed the same way every other section's numbers were fixed: stop
trusting the LLM to format something it can't reliably produce. The
real document title and a working link (both looked up at ingest time
via hal_title_lookup.py, or backfilled for older chunks) are attached
to each validated claim here in Python, and the whole "## Conseils
pratiques" section is rendered deterministically in generate_report()
— Stage 2 is told to omit that section entirely rather than attempt it.
"""
import asyncio
import json
import re
...
from hal_title_lookup import hal_id_from_filename, hal_url, offline_fallback_title

_client = Mistral(api_key=settings.mistral_api_key) if settings.mistral_api_key else None


def _compute_weather_stats(weather: WeatherData) -> dict:
    """
    Authoritative weather aggregates, computed in Python — NOT by the LLM.
    This is the direct fix for the earlier-found bug where Stage 1
    independently averaged the 16-day temperature list itself and got a
    different number (28.6°C) than ml_service.py's own Python-computed
    mean (28.4°C) for the same data. Filters None (trailing forecast
    days Open-Meteo sometimes returns null for), same as ml_service.py.
    """
...
## Rendement estimé
Only include this section if yield_summary.warning is null or if
yield_estimate_q_ha is present — if it is null AND a warning is
present, state the warning plainly instead of the section content.

If yield_estimate_q_ha is present, state it as "- **Rendement estimé** :
{yield_estimate_q_ha} q/ha (fourchette indicative : {yield_range_low_q_ha}
- {yield_range_high_q_ha} q/ha)" followed immediately by method_note
verbatim (do not reword it — it already states its own approximation
caveat precisely). Every number in this section comes directly from
yield_summary — do not recompute, round differently, or adjust any of
them. This is a simplified estimate, not a real agronomic yield
prediction — say so in one short sentence if method_note doesn't
already make that clear enough on its own.

Do NOT write a "## Conseils pratiques" section — omit it entirely from
your output. It is inserted automatically after your output, built
directly from verified source data (a real document title and link per
claim), rather than generated here — you were never given the document
...
def _validate_grounded_claims(raw_claims: list, chunks_by_id: dict[str, RetrievedChunk]) -> list[dict]:
    """
    ...
    Also attaches source_document/title/url to each surviving claim,
    read directly from the matching RetrievedChunk (real data, never
    something the LLM produced) — this is what lets the "## Conseils
    pratiques" section be rendered with a real citation instead of the
    raw chunk_id the model had no way to turn into a proper source.
    """
    validated = []
    for item in raw_claims:
        ...
        if chunk is None:
            continue
        title = chunk.title or offline_fallback_title(chunk.source_document)
        url = chunk.url or hal_url(hal_id_from_filename(chunk.source_document))
        validated.append({
...
_ALERTES_HEADING_RE = re.compile(r"^##\s+Alertes\s*$", re.MULTILINE)
_DONNEES_MANQUANTES_HEADING_RE = re.compile(r"^##\s+Données manquantes\s*$", re.MULTILINE)


def _insert_conseils_pratiques(report_text: str, conseils_section: str) -> str:
    """
    Stage 2 was told not to write "## Conseils pratiques" — insert the
    deterministically-built section at its documented position (right
    before "## Alertes"). Falls back to inserting before "## Données
    manquantes", then to appending at the end, in case the model didn't
    perfectly follow the section order.
    """
    match = _ALERTES_HEADING_RE.search(report_text)
    if match:
        return report_text[: match.start()] + conseils_section + "\n" + report_text[match.start() :]

    match = _DONNEES_MANQUANTES_HEADING_RE.search(report_text)
    if match:
        return report_text[: match.start()] + conseils_section + "\n" + report_text[match.start() :]
...
async def synthesize_stage1(
    ...
    # dl_observation, agro_calc, and yield_estimate are deliberately NOT
    # included in the Stage 1 payload — they never reach the LLM, so
    # there's no path for Stage 1 to "helpfully" reformat, recompute, or
    # misstate them (the exact failure mode found with weather_summary's
    # temperature mean). They're merged straight into the returned
    # SynthesisJSON below as pre-verified data.
    weather_stats = _compute_weather_stats(weather)
    ...
    # this, the entire server (including serving "/" and static assets to
    # every other user) freezes for the whole duration of the LLM call.
    ...
    if dl_observation.source != "unavailable":
        from services.dl_service import crops_roughly_match
    ...
    return SynthesisJSON(
        ...
        yield_summary=yield_estimate.model_dump(),
...
async def generate_report(synthesis: SynthesisJSON, parcel: ParcelResolution) -> AdvisorReport:
    """
    parcel is now passed in separately so Stage 2 can be given the land
    area explicitly — SynthesisJSON doesn't carry it (only location).
    """
    _require_client()
