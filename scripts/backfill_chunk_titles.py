...
already in chroma_store — needed because those chunks were indexed
...
via hal_title_lookup, then updates each chunk's metadata in place —
merging title/url in without touching crop/region/topic.

Usage:
    python backfill_chunk_titles.py            # do it
    python backfill_chunk_titles.py --dry-run  # just show what would change
"""
import argparse
