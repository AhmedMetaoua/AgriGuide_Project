...
class YieldEstimate(BaseModel):
    ...
    project's scope) — an honest placeholder, same trust tier as
    ...
    METHOD's uncertainty, not a statistical confidence interval — see
    method_note.
    """
    crop: str
...
class NdviHeatmapResponse(BaseModel):
    ...
    bounds: Optional[dict] = None  # {"south": float, "west": float, "north": float, "east": float} — WGS84, for L.imageOverlay
    warning: Optional[str] = None
